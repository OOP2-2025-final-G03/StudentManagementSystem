...
| POST | a/auth/logout | ログアウト処理       |
| GET  | /auth/me     | ログイン中ユーザー情報取得 |

**ユーザー管理 API（user）**
| メソッド | パス                | 内容        | 権限            |
| ---- | ----------------- | --------- | ------------- |
| GET  | /user        | ユーザー一覧・検索 | 教師 / スーパーユーザー |
| GET  | /user/detail | ユーザー詳細取得  | 全ユーザー         |
| POST | /user/create | ユーザー登録    | スーパーユーザー      |
| POST | /user/update | ユーザー情報更新  | スーパーユーザー      |
| POST | /user/delete | ユーザー削除    | スーパーユーザー      |

**学生管理 API（user/students）**
| メソッド | パス                | 内容        | 権限            |
| ---- | ----------------- | --------- | ------------- |
| GET  | /user/students | 学生一覧・条件検索 | 教師 / スーパーユーザー |


**科目管理 API（subject）**
| メソッド | パス                   | 内容        | 権限            |
| ---- | -------------------- | --------- | ------------- |
| GET  | /subject        | 科目一覧・条件検索 | 全ユーザー         |
| GET  | /subject/detail | 科目詳細取得    | 全ユーザー         |
| POST | /subject/create | 科目登録      | 教師 / スーパーユーザー |
| POST | /subject/update | 科目情報更新    | 教師 / スーパーユーザー |
| POST | /subject/delete | 科目削除      | 教師 / スーパーユーザー |

**選科管理 API（enrollment）**
| メソッド | パス                       | 内容        | 権限      |
| ---- | ------------------------ | --------- | ------- |
| POST | /enrollment/create  | 学生の選科登録   | 学生      |
| POST | /enrollment/delete  | 学生の選科削除   | 教師      |
| GET  | /enrollment/student | 学生の選科一覧取得 | 学生 / 教師 |
...
| POST | /grade/update  | 成績更新     | 教師 |
| POST | /grade/delete  | 成績削除     | 教師 |

**成績分析 API（analytics）**
| メソッド | パス                            | 内容            |
| ---- | ----------------------------- | ------------- |
| GET  | /analytic/grades/summary | 平均点・最高点・最低点算出 |
| GET  | /analytic/students       | 学生別成績分析       |
| GET  | /analytic/grades/chart   | グラフ表示用データ取得   |
| GET  | /analytic/grades/predict | 未来成績の予測       |
...
## 詳細資料
> ここで細かく各画面の機能や作り方を説明します。(未実装機能と実装計画)
#### ログイン機能
自動ログイン（ログイン状態の保持）」と「パスワードを記憶（ブラウザ機能）」が実装されてない。
- 自動ログイン（ログイン状態の保持）:
    - Flask-Loginの remember=True 機能を使用します。

    - HTMLのチェックボックスに name 属性を追加し、サーバー側でそれを受け取って login_user 関数に渡します。

- パスワードを記憶:

    - これはサーバーの機能ではなくブラウザの機能です。

    -  現在のHTMLにある autocomplete="off" がブラウザの保存機能を阻害しているため、これを修正します。

#### 成績情報および成績に関する機能

#### 科目情報および科目に関する機能
科目と学生を紐付ける機能が存在しないため、「履修登録情報（Enrollment）」機能を新規に実装する。
> 
- 実装すべき機能詳細
    - 中間テーブルの作成: 多対多の関係を管理するため、Enrollment モデルを作成する。

    - 履修登録情報の管理:
        - 科目リストに新しく詳細ボタンを追加する。
        - 詳細ボタンを押下した際、その科目の詳細情報と誰がこの科目を履修しているのか（履修登録情報）のリストを表示する。

重複チェック: 既に登録済みの科目を二重登録できないようバリデーションを行う。

#### ユーザー情報およびユーザーに関する機能
ユーザー表示は静的なダミーデータで行われているため、データベース（SQLite）との完全な連携と権限管理を実装する。

- 実装すべき機能詳細
    - DB連携リスト表示: routes/user.py を修正し、Student および Teacher モデルから select() 文を用いてデータを取得する。

- CRUD処理:
    - 登録: User テーブルだけでなく、認証用の Credential テーブルと Password テーブルへ同時に書き込むトランザクション処理を実装する。

    - 削除: 外部キー制約（on_delete='CASCADE'）を利用し、ユーザー削除時に関連する成績や履修データも整合性を保って削除されるようにする。

- 権限によるビュー制御:
    - current_user.role を判定し、Jinja2テンプレート内で {% if current_user.role == 'admin' %} を用いて、編集・削除ボタンの表示/非表示を切り替える。
    - 管理者（admin）: 全てのユーザー情報を表示し、編集/削除可能。
    - 学生（student）: 自身の成績情報のみ表示。
    - 教員（teacher）: 自身が担当する科目の成績情報を表示。
